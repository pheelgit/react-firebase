...
export const TodoItem = ({ todo }) => {
  ...
  useEffect(() => {
    const checkDate = setInterval(() => {
      if (todo.expired === false && Date.parse(new Date()) > todo.todoDate) {
  ...
  const { handleSubmit, watch, control, setFocus } = useForm()
  ...
  return (
    <ListItem divider className='flex'>
      ...
      <Box className='flex gap-4 '>
        {isUpdating ? (
          <PublishedWithChanges onClick={() => handleUpdate(todo)} />
        ) : (
          <SettingsIcon onClick={() => setIsUpdating(true)} />
        )}
